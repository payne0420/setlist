...
class QueueDialog(QDialog):
    ...
    Template.py), so it sidesteps the frameless, fixed-geometry main window. It
    ...
    def __init__(self, controller):
        ...
        # Qt.Tool gives the dialog its own decorations and keeps it floating
        # above (and recoverable from behind) the frameless, translucent main
        # window — a plain child dialog can get lost there on macOS.
...
class MainWindow(QMainWindow, Ui_MainWindow):
    def __init__(self):
        ...
        # Queue button: opens the non-modal multi-playlist panel. Added
        # programmatically in the free band of the fixed-geometry frame so the
        # generated Template.py is left untouched. (int setGeometry overload —
        # QtCore.QRect is not imported in this module.)
        self.QueueBtn = QPushButton("⬇  Download Queue", self.frame)
        self.QueueBtn.setObjectName("QueueBtn")
        self.QueueBtn.setGeometry(75, 330, 170, 32)
        self.QueueBtn.setCursor(QCursor(Qt.PointingHandCursor))
        self.QueueBtn.setStyleSheet(
            "QPushButton#QueueBtn {"
            " background-color: rgba(255, 255, 255, 28); color: white;"
            " border: 1px solid rgba(255, 255, 255, 90); border-radius: 5px;"
            " font-weight: bold; }"
            "QPushButton#QueueBtn:hover { background-color: rgba(255, 255, 255, 55); }"
            "QPushButton#QueueBtn:disabled {"
            " color: rgba(255, 255, 255, 110); border-color: rgba(255, 255, 255, 40); }"
        )
    ...
    def _start_scraper(self, url, cancel_event, on_item_finished, item_id=None):
        ...
        flow and each queue item.

        ``on_item_finished(status, message)`` is the authoritative terminal
        callback (driven by ScraperThread.item_finished). Thread teardown is
        handled separately in _cleanup_scraper_thread so signals from a finished
        item are disconnected before the next item starts.
        """
    ...
    def exitprogram(self):
        # Cancel and briefly wait on any running download threads so we don't
        # exit with a live QThread (Qt aborts with "QThread destroyed while
        # still running"). The queue makes long multi-item runs common, so a
        # mid-queue quit is a realistic path.
        ...
        for t in list(self._active_threads):
            with contextlib.suppress(RuntimeError):
                if hasattr(t, "request_cancel"):
                    t.request_cancel()
                if t.isRunning():
                    t.wait(3000)  # bounded so the UI can't hang on exit
        sys.exit()
